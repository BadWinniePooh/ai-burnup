...
function CardEditForm({ card, project, allCards, theme, onUpdate, onApiSave, onDelete, readOnly = false }) {
  ...
  const leadTimeDays = local.endDate
    ? Math.max(0.5, window.countWorkdays(local.startedDate || local.createdDate, local.endDate))
    : null;
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginTop: 22 }}>
        ...
        <FormField label="Estimation" theme={theme}>
          ...
          <div style={{ fontSize: 11, color: theme.textSubtle, marginTop: 4, fontFamily: 'ui-monospace, Menlo, monospace' }}>
            {leadTimeDays !== null
              ? <><span style={{ color: theme.success }}>≈ {leadTimeDays.toFixed(1)} working days for charts</span> · actual lead time</>
